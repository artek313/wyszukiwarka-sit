...
const sands = [
	...
	},
	{
		...
		additionalInfo: 'biało czerwony nadruk',
	},
	{
		nr: 34,
		name: 'Monao',
...
searchField.addEventListener('input', function (e) {
	...
	searchSands.forEach(function (el, i) {
		const imageName2 = el.imgName2 ? el.imgName2 : el.imgName2

		const tr = `<tr>
            <td>${el.nr ? el.nr : 'bez numeru'}</td>
            <td>${el.name}</td>
            <td>
						   <img  class="thumbnailImage" src="${el.imgName ? 'img/' + el.imgName : ''}"/>
						   <img  class="thumbnailImage" src="${el.imgName2 ? 'img/' + el.imgName2 : ''}"/>
						</td>
            <td><img  class="thumbnailImage" src="${el.imgOrder ? 'img/' + el.imgOrder : ''}"/></td>
